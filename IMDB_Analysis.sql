...
select year(date_published) as Year , count(*) as number_of_movies from movie group by year(date_published);
...
-- Type your code below:
select country , count(*) as number_of_movies from movie where country in ('USA','India') and year(date_published) = 2019 group by country; 
...
-- Type your code below:
select genre from genre group by genre order by count(*) desc limit 1;
...
select genre , avg(duration) from movie left join genre on movie.id = genre.movie_id group by genre;
...
from movie left join genre on movie.id = genre.movie_id 
group by genre) as T
...
select movie_id , avg_rating , rank() over (order by avg_rating desc) as movie_rank
from ratings
...
from movie left join ratings 
on movie.id = ratings.movie_id where avg_rating > 8) as T1
...
+---------------+-------------------+ */
-- Type your code below:
select genre , count(id) as movie_count from(
select genre , id from movie
left join genre
on movie.id = genre.movie_id
left join ratings
on movie.id = ratings.movie_id
where month(date_published) = 3 and year(date_published) = 2017
and country = 'USA' and total_votes > 1000) as T
group by genre;
...
-- Type your code below:
select count(id) as movie_count from movie left join ratings
on movie.id = ratings.movie_id
...
-- Type your code below:
select case when (select count(total_votes) from movie left join ratings 
on movie.id = ratings.movie_id where country = "Germany") > (select count(total_votes) from movie 
left join ratings on movie.id = ratings.movie_id where country = "Italian") then 'YES' 
else 'NO' end as Ans;
